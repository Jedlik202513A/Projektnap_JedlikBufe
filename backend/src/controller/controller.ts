...
export default class myController implements IController {
	...
	private deleteOne = async (req: Request, res: Response) => {
		try {
			...
			if (refDocuments) {
				res.status(403).send({
					message: `Document with id ${id} has reference in manySide table!`,
				});
			} else {
	...
	private getManyByKeyword = async (req: Request, res: Response) => {
		...
		try {
			...
			const data = await this.many.aggregate([
				...
				{
					$match: {
						$or: [{ 'as_FK_neve.field1': myRegex }, { description: myRegex }],
					},
	...
	private modifyManyPUT = async (req: Request, res: Response) => {
		try {
			const id = req.params.id;
			const body = req.body;
			const modificationResult = await this.many.replaceOne({ _id: id }, body, {
				runValidators: true,
			});
